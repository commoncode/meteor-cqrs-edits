...
updateEditDoc = function (event, template) {
  var $form = template.$(event.currentTarget).parents('.edit-document-form'),
    ...
    applyOT = function (doc, oldValue) {
      ...
      while (oldValue.charAt(start) === value.charAt(start)) {
        start += 1;
      }

      while (oldValue.charAt(oldValue.length - 1 - end) === value.charAt(value.length - 1 - end) && end + start < oldValue.length && end + start < value.length) {
        end += 1;
      }

      if (oldValue.length !== start + end) {
  ...
  case 'textarea':
    ...
    if (value === template.shareDoc.getText()) {
      return;
    }
...
Template.editDocFormText.rendered = function () {
  var template = this,
    $input = template.$('input'),
    shareName = $input.attr('id'),
    oldValue = $input.val();

  sharejs.open(shareName, 'text', location.protocol + '//' + location.host + '/channel', function (error, doc) {
    if (error) {
      console.log(error);
      return;
    }
    ...
    var replaceText = function (newText, transformCursor) {
      ...
      if (transformCursor) {
        newSelection = [
          transformCursor(input.selectionStart),
          transformCursor(input.selectionEnd)
        ];
      }
      ...
      if (newSelection && window.document.activeElement === input) {
        $input.selectRange(newSelection[0], newSelection[1]);
      }
    };
    ...
    doc.on('insert', function (position, text) {
      ...
      oldValue = $input.val().replace(/\r\n/g, '\n');

      return replaceText(
        oldValue.slice(0, position) + text + oldValue.slice(position),
        transformCursor
      );
    });
    ...
    doc.on('delete', function (position, text) {
      ...
      oldValue = $input.val().value.replace(/\r\n/g, '\n');

      return replaceText(
        oldValue.slice(0, position) + oldValue.slice(position + text.length),
        transformCursor
      );
    });
  });
