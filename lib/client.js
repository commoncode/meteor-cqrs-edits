...
operationalTransform = function (doc, oldValue, newValue) {
  var end = 0,
    start = 0;
  ...
  while (oldValue.charAt(start) === newValue.charAt(start)) {
    start += 1;
  }

  while (oldValue.charAt(oldValue.length - 1 - end) === newValue.charAt(newValue.length - 1 - end) && end + start < oldValue.length && end + start < newValue.length) {
    end += 1;
  }
  ...
  if (newValue.length !== start + end) {
    return doc.insert(start, newValue.slice(start, newValue.length - end));
  }
};
