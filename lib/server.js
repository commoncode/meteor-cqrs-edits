    ...
    if(editDoc.state==='committed') {
      ...
      for (var i = updateKeys.length - 1; i >= 0; i--) {
        if(editDiff[updateKeys[i]]!==undefined) {
          ...
          data[updateKeys[i]] = editDoc[updateKeys[i]];
        }
      };
      ...
      if(_.keys(data).length) {
        ...
        var path = apiURL('cqrs/' + editDoc.collection_name + '/' + editDoc.docId + '/');
...
Meteor.methods({
  getOrCreateEditDoc: function (obj, collection_name) {
    ...
    if(editDoc===undefined){
      ...
      editObj.origObj = _.omit(_.pick(obj, _.keys(obj)), ['_id', 'type', 'collection_name']);
