...
EditDocuments.find().observeChanges
  changed: (id, fields) ->
    ...
    diffs = {}  # the working copy
    editDoc = EditDocuments.findOne _id: id  # should be 'this'?
    editDiff = EditDiffs.findOne editDocId: editDoc._id

    if editDiff isnt undefined
    ...
    _.each(_.keys(fields), (element, index, list) ->
      original = editDoc.origObj[element]

      if typeof original is 'object'
    ...
    if editDoc.state is 'committed'
      if editDoc.hasChanges()
        ...
        httpMethod = editDoc.action is 'delete' ? 'DELETE' : 'PATCH'
...
Meteor.methods
  getOrCreateEditDoc: (obj) ->
    ...
    if editDoc and editDocs.count() > 1
      ...
      for doc in dupDocs
        if editDoc._id isnt doc._id
          EditDocuments.remove _id: doc._id
